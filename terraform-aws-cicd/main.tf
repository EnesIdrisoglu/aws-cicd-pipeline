...
resource "aws_codebuild_project" "containerAppBuild" {
  ...
  source {
    ...
    type                = "GITHUB"
    location            = "https://github.com/EnesIdrisoglu/aws-cicd-pipeline.git"
    git_clone_depth     = 0
  }
}
...
resource "aws_codestarconnections_connection" "github-pipeline" {
...
resource "aws_codepipeline" "python_app_pipeline" {
  ...
  stage {
    ...
    action {
      category = "Source"
      configuration = {
        ConnectionArn    = aws_codestarconnections_connection.github-pipeline.arn
        FullRepositoryId = "EnesIdrisoglu/aws-cicd-pipeline"
        BranchName       = "master"
      }
      input_artifacts = []
      name            = "Source"
